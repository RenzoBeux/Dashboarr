...
export interface AdguardTopBlockedSettingsValue extends Record<string, unknown> {
  instanceIds: InstanceBindingValue;
  maxItems: number;
  hideWhenEmpty: boolean;
}

export const ADGUARD_TOP_BLOCKED_DEFAULT_SETTINGS: AdguardTopBlockedSettingsValue = {
  instanceIds: INSTANCE_BINDING_ALL,
  maxItems: 5,
  hideWhenEmpty: false,
};

export function AdguardTopBlockedSettings({ slotId }: WidgetSettingsComponentProps) {
  const { settings, update } = useWidgetSettings<AdguardTopBlockedSettingsValue>(
    slotId,
