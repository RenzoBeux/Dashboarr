...
/**
 * Random bytes source. The app binds expo-crypto (lib/config-crypto.ts); the
 * backend's web editor binds `crypto.getRandomValues`. Injected rather than
 * imported so this module stays free of Expo and works in a browser bundle,
 * including plain-http LAN origins where WebCrypto's `subtle` is unavailable
 * but `getRandomValues` is not.
 */
...
export function generateSaltHex(rng: Rng): string {
  return bytesToHex(rng(SALT_LENGTH));
}
 ...
 * usual.
 */
export function encryptJsonStringWithKey(
  plainJson: string,
  key: DerivedKey,
  rng: Rng,
): EncryptedEnvelope {
