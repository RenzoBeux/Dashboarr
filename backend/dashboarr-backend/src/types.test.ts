...
test("rtorrent, transmission, lidarr, bindery, jellystat, unraid, jackett, autobrr and cleanuparr are accepted kinds", () => {
  for (const kind of ["rtorrent", "transmission", "lidarr", "bindery", "jellystat", "unraid", "jackett", "autobrr", "cleanuparr"]) {
    const result = configPayloadSchema.safeParse({
