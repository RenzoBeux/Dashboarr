...
- **Keep the site in sync** when adding/removing a service, changing major features, or updating download links. The landing page states counts (currently "26 services", "31 widgets", plus a spelled-out "Twenty-six integrations" above the services grid) — update them alongside `SERVICE_IDS` and `DASHBOARD_WIDGET_IDS` in `lib/constants.ts`.
