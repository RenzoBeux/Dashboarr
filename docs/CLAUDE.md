...
- **Keep the site in sync** when adding/removing a service, changing major features, or updating download links. The landing page states counts (currently "20 services", "26 widgets") — update them alongside `SERVICE_IDS` and `DASHBOARD_WIDGET_IDS` in `lib/constants.ts`.
